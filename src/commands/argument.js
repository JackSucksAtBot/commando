...
class Argument {
	...
	async obtain(msg, value, promptLimit = Infinity) {
		...
		while(!valid || typeof valid === 'string') {
			...
			const argsprompt = new Discord.RichEmbed()
			.setDescription(`**__${this.prompt}__**\n\nYou may respond with \`cancel\` to cancel this command.\nThe command will be automatically cancelled in 30 seconds.`)
			.setColor(msg.guild.me.displayColor)
			.setFooter(`Requested by ${msg.author.username}#${msg.author.discriminator}`,`${msg.author.avatarURL}`)
			.setTimestamp();

			prompts.push(await msg.reply(argsprompt));
