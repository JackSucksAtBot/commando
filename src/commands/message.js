...
class CommandMessage {
	...
	async run() { // eslint-disable-line complexity
		...
		if(this.command.guildOnly && !this.message.guild) {
			...
			const guildOnlyEMBED = new Discord.RichEmbed()
			.setDescription(`${emojis.no} **Guild Only** - This command may only be used in a guild/server.`)
			.setColor(colours.error);
			return this.embed(guildOnlyEMBED) //reply(`The \`${this.command.name}\` command must be used in a server channel.`);
		}

		if(this.command.nsfw && !this.message.channel.nsfw) {
			this.client.emit('commandBlocked', this, 'nsfw');
			const nsfwOnlyEMBED = new Discord.RichEmbed()
			.setDescription(`${emojis.no} **NSFW Only** - This command may only be used in an NSFW channel.`)
			.setColor(colours.error);
			return this.embed(nsfwOnlyEMBED); //reply(`The \`${this.command.name}\` command can only be used in NSFW channels.`);
		}
		...
		if(!args && this.command.argsCollector) {
			...
			if(result.cancelled) {
				...
				const cmdargscancel = new Discord.RichEmbed()
				.setDescription(`${emojis.yes} **Success** - Cancelled command.`)
				.setColor(colours.success);

				return this.message.channel.send(cmdargscancel);
			}
			args = result.values;
		}
